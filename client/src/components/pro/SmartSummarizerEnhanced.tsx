...
import { getUserCourses, getUserAssignments, uploadFile, saveSummary, type Course, type Assignment } from "@/lib/firebase";
import { useLocation } from "wouter";
import * as pdfjsLib from 'pdfjs-dist'; // Only this import, no worker import!
...
export default function SmartSummarizerEnhanced({ isOpen, onClose }: SmartSummarizerEnhancedProps) {
  ...
  Return your response in this JSON format and NOTHING ELSE. Do not include any explanation, code block, or text outside the JSON:
  {
  ...
  ${text.substring(0, 4000)}`;
    ...
    // --- Robust JSON Parsing Section ---
    try {
      // Try direct JSON parsing first
      return JSON.parse(content);
    } catch (parseError) {
      // Try to extract JSON substring if there is extra text or code block
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
          return JSON.parse(jsonMatch[0]);
        } catch (innerErr) {
          // If still fails, show the full AI response
        }
      }
      ...
      throw new Error(
        'Failed to parse AI response. Here is what the AI returned:\n\n' +
        content +
        '\n\nYou may need to adjust your prompt or try a different model.'
      );
    }
  };
...
${text.substring(0, 1000)}`;
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      ...
      // Only add fields if they are defined (avoid undefined)
      ...
      if (selectedAssignmentId) summaryData.assignmentId = selectedAssignmentId;
      if (documentUrl) summaryData.originalDocumentUrl = documentUrl;
      if (documentName) summaryData.originalDocumentName = documentName;
